...
import { sendVerificationEmail, sendResetPasswordEmail } from "../utils/services/sendVerificationEmail";
...
export const changePassword = async (req: Request, res: Response) => {
	try {
	  ...
	  const salt = await bcrypt.genSalt(10);
	  const hashed = await bcrypt.hash(req.body.newPassword, salt);
	  const userUpdate = await UserInstance.update({password: hashed},{where:{id: userId.id} });
